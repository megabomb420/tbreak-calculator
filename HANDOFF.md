...
Shipped to `main` as `e7c4904` and published by `.github/workflows/pages.yml`, which re-ran `npm ci`, `npm test`, `npm run test:tz`, `npm run typecheck` and `npm run build` on Node 24 before `actions/deploy-pages`; both jobs succeeded (run 36020388972). The same release then took two follow-up commits on `main` (the two-destination navigation, the open timeline, the compact **Update last use** button, and the even two-tab bar); each was published by the same workflow and mirrored to Cloudflare with `BASE_PATH=/ npm run build && npx wrangler pages deploy dist --project-name tbreak-calculator --branch main`, and the live-verify figures below are from the final tree.
...
- Navigation is exactly **Today** and **History**, each 183px wide at 390px with a 24px gap between them and the selected pill filling its half; **Start a new calculation** opens the plan sheet (four goals, **Choose my break length**, the draft, **View saved plan**) over an inert background.
