...
- Settings → About reads **0.38.0** on both origins; the served bundle (`index-BPE6OLVZ.js` / `index-DTVmcUoJ.css` on Pages, `index-CJS8ym6i.css` on the mirror) and the viewport meta (`width=device-width, initial-scale=1, viewport-fit=cover`, no `maximum-scale`) match the release.
