...
- Settings → About reads **0.38.0** on both origins; the served bundle (`index-BPE6OLVZ.js` / `index-DTVmcUoJ.css` on Pages, `index-CJS8ym6i.css` on the mirror) and the viewport meta (`width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover`) match the release, and the document computes `touch-action: pan-x pan-y` on `html` and `body`.
