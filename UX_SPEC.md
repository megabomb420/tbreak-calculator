...
| `active-break` | attempt `active` | Day/target hero, immediate no-use check-in with Undo (the receipt line and Undo appear only once the day is recorded; **How are you feeling?** is a text action, not a second CTA), and a quiet recorded-days line. Above the hero sits the `Your plan` \| `Recovery outlook` switch when the running break owns a frozen record: it swaps the day/target block for that record's own outlook panel, and both modes keep the check-in and the guidance below unchanged. Then **one support card** (`data-testid="support-card"`): the hardest rated topic of the last 48 hours, the person's own replacement action when their saved plan covers a routine, an urge or an empty evening, or the guide's first step otherwise, with the full guide (explanation, remaining steps, what to avoid, seek-help and sources) open beneath it. Every area rated 4 or harder on the oriented scale is one tap away as an alternative topic inside the same card; a day whose ratings select nothing raises no topic and defaults to the day's practice area with the line "Rate how you feel if this is not the problem.", never two stage-default essays. Nothing else is collapsed: **This stage** (label, headline, what is commonly noticed, the day's practical activity), **Experiences** (the Reddit carousel), **Your recorded changes** when check-ins allow it, **Your plan for urges** (the cue/alternative editor: the draft is held locally, **Save plan** writes it through the app's single preparation writer, the if–then lines it builds are shown as they are typed, and **Remove plan** clears it) and **Your break timeline** (the shared journey plus the research note) all render inline. Footer: **End break early**; **Mark complete** appears in the action zone from the target instant. No THC-use log appears here. |
