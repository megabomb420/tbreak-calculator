...
| `active-break` | attempt `active` | Day/target hero, immediate no-use check-in with Undo (the receipt line and Undo appear only once the day is recorded; **How are you feeling?** is a text action, not a second CTA), and a quiet recorded-days line. Then **one support card** (`data-testid="support-card"`): the hardest rated topic of the last 48 hours, the person's own replacement action when their saved plan covers a routine, an urge or an empty evening, or the guide's first step otherwise. Every area rated 4 or harder on the oriented scale is one tap away as an alternative topic inside the same card; a day whose ratings select nothing raises no topic and defaults to the day's practice area with the line "Rate how you feel if this is not the problem.", never two stage-default essays. The full guide, its sources and its seek-help line live inside the card's **More**. Closed disclosures hold the stage (`This stage`: label, headline, what is commonly noticed, the day's practical activity), the Reddit experiences (`Experiences`) and the recorded differences. **Edit your plan** / **Set a plan for urges** opens the stored trigger/replacement plan in a sheet. Footer links: **Full timeline** (a sheet with the shared journey and the research note), **Recovery outlook** when the running break owns a frozen record (a sheet with that record's own outlook), then **End break early**; **Mark complete** appears in the card's action zone from the target instant. No THC-use log appears here. |
...
- Practical daily advice is **one support card** and renders before the timeline; the stage, the experiences and the differences behind it are closed disclosures. See §17 for the current advice selection and content contract.
- The live journey is opened from the footer's **Full timeline** as a sheet (§9.7): Start → evidence phases → Target, with past-day check-in markers, the current leg marked "You are here", and future legs kept as expectations. Each leg's "may notice" / "can help" expectations sit behind its disclosure.
- The break's own frozen **Recovery outlook** (§9.1) stays reachable while it runs: a **Recovery outlook** footer action opens a sheet rendering the same panel as the result screen — same content, no new science. A chosen-duration plan or an open-ended tracker owns no calculation record, shows no such action and renders no disclosure.
...
Today prioritises what the person can do now: direct check-in with correction, optional ratings, one advice topic and one activity for the day. Phase context sits in a closed **This stage** disclosure instead of the first screen, so the card stays a daily helper. Phase windows remain population patterns. A practical day-specific activity is explicitly an editorial schedule, not a symptom prediction. Clinical/self-care source links live in each guide's **More** panel and in Science. The full journey is a sheet opened from **Full timeline** rather than preceding today's advice.

Recent ratings are the only input to advice selection, and no stored preference exists. Every area rated 4 or harder on the oriented scale is offered, ordered by severity: the hardest leads the one card and each other area is one tap away inside it, and each topic names the dated rating it came from. A day whose ratings select nothing raises no topic and defaults to the day's practice area, and an area rated as comfortable is never offered as a problem. No fresh ratings is a distinct state, not zero symptoms. A manual browser (**Not this?**) exposes all eleven topics and changes nothing. When the stored plan covers a routine, an urge or an empty evening, the person's own replacement action is the card's action line, with the flagged triggers and the fallback plan beneath it. The same component serves chosen/calculated breaks and open-ended tracking.
...
A manually controlled carousel draws from 17 reviewed paraphrase cards across 12 r/Petioles discussions. Every card is tagged to one or more current break windows (`days_1_3`, `days_2_6`, `days_7_14`, `days_14_21`, `days_21_28`, `beyond_28`); Today displays at most five cards from the active window behind the **Experiences** disclosure and ranks cards matching the currently selected sleep/craving/appetite/etc. areas first. Each card shows the reported period, uses the label **From Reddit · Personal experiences**, states that it is not a prediction and links to the original discussion. No medical treatment claim is sourced to Reddit. Curated text remains available offline; links require a connection.
