...
An accessible **“Your plan” | “Predicted reset”** segmented control (default “Your plan”; tablist semantics) selects between two modes. “Your plan” is the reading screen below; “Predicted reset” renders the versioned non-engine recovery outlook (`tolerance-recovery-outlook-v1`, CALCULATOR_SPEC §7.11; copy module `src/ui/recovery-copy.ts`) in the fixed hierarchy described in the 0.9.0 revision note. “Your plan” cards, in scroll order:
...
The recovery-outlook (“Predicted reset”) content is separately versioned as `tolerance-recovery-outlook-v1` and mapped by a second template module, `src/ui/recovery-copy.ts` — see the 0.9.0 revision note above and `EVIDENCE_CONTENT_SPEC.md`. Codes absent from either module render nothing.
