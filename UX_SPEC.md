...
## 17. Practical daily support (0.22.0; guide contract 0.38.0; support topics 0.38.0)
...
**Support topics.** A sheet asks **What would you like help with?** once after a break calculation finishes (never after a drug-test question, and never again once topics are stored), and Today's footer keeps it one tap away on the right, opposite **Start a new calculation**, showing the count of chosen topics. It offers the same eleven topics in four groups, multi-select, and writes nothing until **Save topics**; the close action leaves the previous choice exactly as it was. What the choice does is stated on the sheet: recent check-in ratings take priority, the chosen topics take turns between days in the person's own order, and every guide stays openable at any time. So the day's precedence is rating → chosen topics (rotating) → the day's own practice, with the target-day review keeping its day; and when a chosen topic or a rating leads the card, the day's own task appears inside that topic's depth as **Today's own task**, so a stored preference can never make the day's activity disappear. The choice is stored in `tbreak.companion-personalisation.v2` (backup family: support areas), never reaches the tolerance or detection engines, and never invents a symptom: choosing a topic only decides which guide leads.
