...
| `active-break` | attempt `active` | Day/target hero, immediate no-use check-in with Undo (the receipt line and Undo appear only once the day is recorded), and a quiet recorded-days line. Above the hero sits the `Your plan` \| `Recovery outlook` switch when the running break owns a frozen record: it swaps the day/target block for that record's own outlook panel, and both modes keep everything below unchanged. Then **This stage** (label, headline, what is commonly noticed, the day's practical activity), then **How to deal with?** — a picker over all eleven topics — with the chosen topic's guide directly beneath it: title, the reading that named it (or "For this stage of the break"), one action line (the person's own replacement first when their plan covers that topic), the flagged triggers and fallback when a plan exists, then the full guide with its steps, avoid line, seek-help line and sources. One topic renders at a time; picking another replaces the block. Nothing else is collapsed: **Experiences** (the Reddit carousel), **Your recorded changes** when check-ins allow it, and **Your break timeline** (the shared journey plus the research note) all render inline. Footer: **End break early**; **Mark complete** appears in the action zone from the target instant. No THC-use log appears here. |
