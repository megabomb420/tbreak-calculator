...
const TARGET_LIMITATION = 'heuristic_duration_target_within_range_v2';
...
function resultOf(profile: UseProfileInput): ToleranceResult {
  return calculateTolerance(profile, TOLERANCE_POLICY_V2, C0);
}
...
describe('current-pattern duration moves the planning target inside the evidence range (tolerance-v2)', () => {
  ...
  it('daily + multiple sessions keeps 21–28 with the intensity drivers; recent never widens the range', () => {
    ...
    assert.equal(recent.preferredTargetDays, 21);
    assert.ok(recent.limitations.includes('heuristic_frequency_intensity_v1'));
    assert.ok(recent.limitations.includes(TARGET_LIMITATION));
  });
  ...
  it('long-established use never raises the target above the evidence range', () => {
    const cases: ReadonlyArray<{ useDays: number; expectedRange: { min: number; max: number } }> = [
      ...
      { useDays: 25, expectedRange: { min: 14, max: 21 } },
  ...
  it('previous-break history stays descriptive and never moves the range or target', () => {
    const history = [
      {
        id: 'b1',
        durationDays: 14,
    ...
    assert.equal(recent.kind, 'tolerance_result');
    assert.equal(established.kind, 'tolerance_result');
    if (recent.kind !== 'tolerance_result' || established.kind !== 'tolerance_result') return;
    assert.deepEqual(recent.recommendedRangeDays, recentWithoutHistory.recommendedRangeDays);
    assert.equal(recent.preferredTargetDays, recentWithoutHistory.preferredTargetDays);
...
describe('frozen historical records stay immutable', () => {
  it('renders a tolerance-v1 record with its stored top-of-range target, never recomputing it', () => {
    ...
    assert.equal(view.kind, 'tolerance_result');
    if (view.kind !== 'tolerance_result') return;
    // Stored numeric output survives verbatim under the v2 presentation.
...
describe('questionnaire routing for current pattern duration', () => {
  ...
  it('sessions/products/routes stay restricted to the 16–30 band where they can change the range', () => {
    // A 4–15 band user is never routed to Q4/Q5 because neither the range rule
    ...
    assert.equal(resolvedPath({ goal: 'tolerance_reset', thcUseDaysLast30: 15 }).includes('Q5'), false);
  });
