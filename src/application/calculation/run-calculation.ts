...
export function runCalculation(snapshot: RawAnswerSnapshot, now: Instant): ResultView {
  ...
  const result = calculateTolerance(snapshot.profile, TOLERANCE_POLICY_V3, now);
  if (result.kind === 'not_applicable' && result.withdrawal === null) {
    ...
    if (instant !== null) {
      return presentToleranceResult(
        {
          ...result,
          withdrawal: computeWithdrawalDisplay(instant, now, TOLERANCE_POLICY_V3.withdrawalAnchors),
        },
        snapshot.profile,
      );
    }
