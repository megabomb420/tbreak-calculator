...
export interface CheckInProps {
  ...
  readonly day: number;
  /** Today's stored report, so the ratings open on what was saved — a rating
   * can be changed or cleared instead of being untouchable once saved. */
  readonly recorded: DailyCheckin | null;
  readonly onSymptomsSave: (symptoms: SymptomValues, note: string | null) => void;
  readonly onClose: () => void;
}

export function CheckInFlow({ day, recorded, onSymptomsSave, onClose }: CheckInProps) {
  const rootRef = useRef<HTMLDivElement>(null);
  useFocusTrap(true, rootRef, onClose);
  const [busy, setBusy] = useState(false);
  const [symptoms, setSymptoms] = useState<SymptomValues>(() => ({
    craving: recorded?.craving ?? null,
    sleep: recorded?.sleep ?? null,
    irritability: recorded?.irritability ?? null,
    anxiety: recorded?.anxiety ?? null,
    appetite: recorded?.appetite ?? null,
  }));
