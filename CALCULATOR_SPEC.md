...
Version: 0.2.0 (spec); Tolerance policy line: **tolerance-v3** (app 0.38.0)
...
Release note (0.9.2): Recovery Outlook v2 adds an explicit, profile-sensitive **predicted tolerance recovery window** beside the unchanged tolerance-v3 plan and the separate Day-28 human CB1 reference. The highest-burden class may reach a 28–42-day product estimate; any part beyond Day 28 is lower-directness, indirect/preclinical-supported product heuristic, not a validated human endpoint. New records freeze the outlook version; records without it retain v1 display semantics. **Retired in 0.38.0 — no window is displayed; see section 7.11.**
...
Scope: deterministic v3 Tolerance Engine, active reduction (cut-down) tracking, qualitative v1 Detection Engine, nominal THC calculation, validation, break mechanics, the retired recovery-outlook version markers retained for stored records (section 7.11), and future scientific extension boundaries — runtime AI stays excluded (unchanged)

Release note (0.17.0): no numeric policy changes. Saved results are not revalidated against a later month; only live elapsed-time guidance advances. Ongoing plans resolve the profile from their owning calculation. Presentation calls ranges planning rules and labels the secondary view Recovery outlook, explicitly stating the lack of clinical validation of individual timelines. **That secondary view was retired in 0.38.0; see section 7.11.**

Release note (0.37.0): numeric policies unchanged. Daily support presentation is `daily-support-v4`; current check-ins do not collect symptom ratings, while legacy rating semantics remain supported. Last-use correction reuses the existing break transitions. Deleted calculations are not re-derived from orphaned snapshots.

Release note (0.38.0): numeric policies unchanged. **The recovery outlook is retired from display.** `tolerance-recovery-outlook-v1` / `-v2` no longer produce a predicted recovery window anywhere in the app: the personalised window could not be defended on the evidence and, shown as a large “estimated recovery window” beside the plan target, it read as a prediction rather than a research reference. `src/domain/recovery/recovery-outlook.ts` now keeps only the two version constants, `BIOLOGICAL_REFERENCE_DAYS = 28` and `hadRecoveryOutlook()`; the window builders and the check-in-facts presenter are deleted, and new calculation records no longer write `recoveryOutlookVersion`. Stored records that carry the field stay valid and are never recomputed or rewritten — the result's always-visible research section explains them with one legacy line and shows no window. The four-week human CB1 PET figure remains a population research reference, presented in that research section and in Science, never as a personal finish line. Practical advice moves to `daily-support-v5`: all eleven guides carry an explanation, five to eight steps, “What to avoid”, “When to get advice” and sources, rendered always open, with experiences matched to the selected topic and capped at eight. See section 7.11. Release notes written before this one that mention Recovery Outlook describe the layer this release retired.
...
- a “one dab/vape = +N break days” penalty — tracked use never adds days to or rewrites a recommendation; a new recommendation comes only from an explicit user recalculation on an updated profile.
- a “100% reset” or “fully reset” claim, and any reset/detox percentage in any research or plan content (section 7.11) — the research section emits population reference wording and time markers, never a percentage and never a personal window.
...
12. For `goal = reduction`, earlier releases asked the user to choose `breakRequested`. **Superseded:** the shipped questionnaire never asks it — `Q2R` survives only as a parseable legacy step id and the snapshot derives the field from the goal. The reduction-with-a-break routing referenced by rules 6–7 and section 7.4 stays defined for legacy input only; the shipped reduction flow always takes the cut-down tracker (section 10.1).
...
18. `goal = reduction` neither requires nor collects `lastUseAt`; rule 6 does not apply to the shipped cut-down route (legacy reduction-with-a-break input is covered by rule 6).
...
- `reduction` (the shipped cut-down route): return `planning_only`; do not invent a break duration. The questionnaire never asks `breakRequested`, so this is the only reduction route the app produces. The historical `reduction` with `breakRequested = true` range path stays defined for legacy input only (rules 6–7, 12).
...
`BreakOutlookV1` is a deterministic derivation from EvidenceGuidanceV1 windows plus optional exposure context. Result and Today MUST reuse this one source. UI MUST NOT invent a second science-copy implementation.
...
### 7.11 Recovery outlook — retired from display (0.38.0)

The recovery outlook is **retired from display**. `tolerance-recovery-outlook-v1` and `tolerance-recovery-outlook-v2` no longer produce any predicted recovery window, interpretation or timeline in the app. The section is retained because the version markers are still part of the stored-record contract.

- **What remains in code.** `src/domain/recovery/recovery-outlook.ts` keeps only the two version constants (`RECOVERY_OUTLOOK_V1_VERSION`, `RECOVERY_OUTLOOK_VERSION`), the `RecoveryOutlookVersion` type, `BIOLOGICAL_REFERENCE_DAYS = 28` and `hadRecoveryOutlook()`. The window builders, the presentation that rendered them, and the check-in-facts presenter were deleted; the v2 heuristic window table is gone from the codebase and no longer appears in this section.
- **Why it was retired.** The v2 window was a product heuristic: its post-28 portion had no validated human support, and it could not be defended as a personalised prediction. Rendered as a large “estimated recovery window” beside the plan target, it read as exactly the personal prediction section 2 prohibits. The app therefore no longer estimates a personal recovery window or recovery date at all.
- **Storage contract.** New calculation records do not write `recoveryOutlookVersion`. A stored record that carries it (v1 or v2) remains valid and is never recomputed, migrated, upgraded or rewritten; its range, target, drivers and policy version stay exactly as saved, and no window is shown for it. Its presence is explained by one honest line in the result's always-visible research section (`legacyOutlookNote`: the record was calculated when the app also showed an estimated recovery window, that estimate was a product heuristic rather than a validated human timeline, and the stored planning target and range are unchanged).
- **The four-week reference.** Day 28 remains `BIOLOGICAL_REFERENCE_DAYS` — the strongest direct human CB1 PET **population research reference** used here. It is presented only as a research reference, in the result's research section (“What the research can and cannot say”) and in Science, with what it does not mean stated beside it. It is never a personal finish line, reset date or target.
- **No numeric effect, before or after.** The outlook never changed `recommendedRangeDays` or `preferredTargetDays`, and retiring it changed no tolerance policy version and no numeric rule: sections 7.3, 7.4 and 7.5 are untouched.
...
- **User limits.** A reduction plan stores `ReductionLimits` — a maximum number of THC-use days in a rolling 7-day window and a maximum number of sessions per use day — plus optional `ThcStrategy` flags (`avoidConcentrates`, `lowerPotency`, `lowerAmount`). They are the user's own precommitments, never engine science. If the app suggests starting limits, the suggestion is a **labelled product heuristic** derived from the user's current pattern: one clear step below the reported weekly use-day rate (`suggestedDays = max(1, min(7, weeklyUseDayRate(useDaysLast30) - 1))`, `weeklyUseDayRate = ceil(useDaysLast30 / 30 × 7)`) and one below the reported sessions per use day with a floor of 1 (`max(1, sessionsPerUseDay - 1)`). Suggestions are always editable and MUST NOT be described as a “safe THC amount”.
- **Events equal sessions.** Each logged THC-use event stores a UTC instant, the UTC offset in force where it was logged, one product and one route. Events are grouped by the user's **local calendar day** (the instant plus that event's own offset, so neither a timezone change nor a daylight-saving change can move a past session onto another day, duplicate it or make it disappear). Several events on one day raise that day's session count but never the use-day count; a new local day with at least one event raises the use-day count. Rows written before 0.29.0 carry no offset of their own and are grouped with the user's current offset — the behaviour they were already grouped by.
- **Derived plan state.** From events, limits, the current UTC offset (the fallback for rows stored without one), and `now`, the engine derives: rolling use-days over the last 7 local calendar days ending today, today's sessions, whether each limit is exceeded, the strategy-exceeded flag (concentrate logged while the plan says avoid concentrates), and the set of breach days. Same events + same clock ⇒ same state.
- **Review rule (explicit product rule, never a biological reset claim).** A day whose sessions exceed the session cap, or that pushes the rolling use-day count over the weekly cap, is one breach day. Two **distinct** breach days inside the rolling 7-day window set the plan to `review_recommended` and show the shipped review prompt: “Your limits were crossed on two days in this rolling week. Adjust the plan if it was unrealistic, or pause it if you want some space.” The state is derived and reversible: when breach days age out of the window the plan returns to `active` automatically. `paused` and `ended` are user-controlled.
...
- **Adaptive tolerance recalculation — retired (0.25.0).** The 0.8.0 path re-ran tolerance-v3 on an observed profile derived from tracked events and froze a NEW `CalculationRecord`, asking for a minimal prefilled refresh when tracked history did not span 30 days. It is no longer shipped: the reduction refresh sheet, the in-card trajectory and the recalculation caller were removed in 0.25.0, so tracked use never generates or rewrites a calculation record and frozen records stay immutable. A new recommendation comes only from an explicit user recalculation with updated inputs; the tracker remains a behavioural commitment, never a calculation.
...
Runtime generative AI is intentionally out of scope. This specification defines no runtime model, no model identifier, no provider inference layer, no AI consent or retention requirements, no AI response schema, and no runtime prompt infrastructure. User-facing explanations, the research section's reference copy, evidence summaries, and personal-history insights are deterministic and derived from reviewed structured data stored or computed locally.
...
- **Goal semantics — resolved (updated 0.25.0):** the shipped questionnaire never asks `breakRequested`; the snapshot derives it from the goal (`Q2R` survives only as a parseable legacy step id), and the reduction goal takes the cut-down tracker rather than a range request (rules 6–7, 12; section 7.4). Abstinence has no finite break range.
