...
- `companion-personalisation-v3` remains independent of use-profile calculations: it selects daily advice only, and is never passed to tolerance-v3 or the outlook constants. It carries the topics the person asked for (`supportAreas[]`, always in taxonomy order), the break they were confirmed for (`forBreak` + `confirmedDay`, null while no break is in hand) and the topic picked by hand for one break day (`pick`). The sheet writes it, Today reads it through `breakFocus(record, breakId, day)` — which returns the confirmed areas for this break, or the stored list as `reusable` when it belongs to another one — and it is migratable on load and part of the backup format, which reads the device-wide v2 list and the v1 focus forward.
...
- `urge-sessions-v1` (`tbreak.urge-sessions.v1`, IndexedDB store `urgeSessions`, IndexedDB schema 2) records the delay timer: the start, the chosen length and, when the person finishes it, the end and their one-word report. A row with no end is a running timer and at most one may exist; `pruneUrgeSessions` drops a timer that was never finished two hours after its own end, so nothing half-done can inflate a later count. The module and `domain/urges/urge-session.ts` are pure and never interpret a session: no function turns a count into a claim about tolerance, withdrawal or control.
- `checkin-reminder-v1` (`tbreak.checkin-reminder.v1`, IndexedDB store `settings` under the id `checkin-reminder`) stores one local time and an on/off flag. `domain/reminders/checkin-reminder.ts` decides whether today's time has passed with no check-in recorded in the running segment; the app raises at most one local notification per local day, and only while it is alive — there is no push subscription and no server, so nothing promises a notification a closed app cannot deliver.
- `infrastructure/notifications.ts` is the only place that touches the Notification API, and every failure there is silent: a reminder is convenience, never a dependency.
- `daily-support-v8` renders the editorial practice by default, with hand-picked topic, confirmed support-topic, target-day, legacy-rated topic and personal-plan precedence. Manual topics are local UI state, reset by break day; no ratings are collected. The support sheet (`companion-personalisation-v3`) stores the topics the person asked to be helped with, bound to the break they were confirmed for, and they take turns by break day from the day they were confirmed; the day's own task then stays inside the leading topic's depth. All eleven guides keep the action and its explanation on the page, with the remaining steps, "What to avoid", sources and, where one applies, "When to get advice" behind one collapsed row; the Experiences carousel is matched to the selected topic and capped at eight.
- The day's advice card carries the delay timer entry (`Ride it out`, resumable from the stored start), and History lists finished sittings under their own section; a stopped or unfinished timer leaves no row.
- Today's stage, advice and experiences are open sections; the break timeline and the guide's remaining steps and `Manage break` are the single-row disclosures. The stage block renders the window label, headline and one sentence only — the window's symptom list renders once, on the journey leg the person is on. `Manage break` reuses atomic suspend/confirm orchestration; opening/canceling never persists a paused intermediate state.
...
Companion personalisation is a stored preference record that selects daily advice and nothing else. Its `supportAreas[]` is migrated forward on load and carried by the backup so a legacy device keeps its data and its topics; it is bound to one break at a time, so a break with no topics confirmed for it reads the day's own practice. The calculation coordinator never receives this record. Check-ins continue to store only the user's current-day experience.
